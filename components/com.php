...
function array2Table($array,$head=array(),$crud=false){
    $html = '<table class="table">';
        ...
        foreach( $array as $key=>$value){
            $html .= '<tr>';
            $html .='<td ><strong>' . strtoupper(htmlspecialchars($key+1)) . '</strong></td>';
            ...
            if($crud==true){
                    ...
                    $id = $vnum[0];
                $html .= '<td >';
                $html .= '<a class="btn btn-sm btn-warning mr-1" href="?'._ROUTE.'&ac=edit&id='.$id.'" ><i class="fa fa-edit"></i></a>';
                $html .= '<a class="btn btn-sm btn-danger mr-1" href="?'._ROUTE.'&ac=delete&id='.$id.'"><i class="fa fa-trash"></i></a>';
                $html .= '</td>';
            }
